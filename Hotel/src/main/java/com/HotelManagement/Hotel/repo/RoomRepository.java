...
//    @Query("SELECT DISTICT r.roomType FROM Room r")
//    List<String> findDistinctRoomTypes();
//
//    @Query("SELECT r FROM Room r WHERE r.id NOT IN (SELECT b.room.id FROM Booking b)")
//    List<Room> getAllAvailableRooms();
//
//    @Query("SELECT r FROM Room r WHERE r.roomType LIKE%:roomType% AND r.id NOT IN(SELECT bk.room.id FROM Booking bk" + "(bk.checkInDate <= :checkOutDate) AND (bk.checkOutDate >= :checkInDate))")
